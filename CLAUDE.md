...
- Content structure follows standard Jekyll conventions:
  ...
  - `_data/nav.yml`, `_data/nav_home.yml` — drive the site navigation (list of `title`/`url` pairs). Note `nav_home.yml` currently has a formatting bug where two YAML list entries are concatenated onto one line (`url: /writing- title: About`) — worth fixing if touching navigation.
...
- There is no local Jekyll build/serve setup checked into the repo (no `Gemfile`). To preview locally you'd need to install Jekyll/Bundler yourself and run the usual `bundle exec jekyll serve`; there's no repo-specific override of that workflow.
...
- `site/en/tutorials/chat_quickstart.ipynb` — a chat quickstart tutorial notebook, despite living under the `site/` directory it is not part of the Jekyll build (Jekyll ignores `.ipynb` files by default).
